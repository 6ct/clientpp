...
error::error(const std::wstring &msg)
    : runtime_error("Use wwhat"), msg(msg), error_code(0), error_msg(L"") {}

error::error(const std::wstring &msg, DWORD error_code)
    : runtime_error("Use wwhat"), msg(msg), error_code(error_code),
...
bool named_proxy_policy::check_policy(
    const WINHTTP_CURRENT_USER_IE_PROXY_CONFIG &config) {
  return config.lpszProxy;
}

WINHTTP_PROXY_INFO named_proxy_policy::make_proxy_info(
...
bool auto_config_url_policy::check_policy(
    const WINHTTP_CURRENT_USER_IE_PROXY_CONFIG &config) {
  return config.lpszAutoConfigUrl;
}

WINHTTP_PROXY_INFO auto_config_url_policy::make_proxy_info(
    session &http_session, const url &dest_url,
    const WINHTTP_CURRENT_USER_IE_PROXY_CONFIG &config) {
  WINHTTP_PROXY_INFO proxy_info = {};
  ...
  if (WinHttpGetProxyForUrl(http_session, dest_url.get_text().c_str(), &opt_pac,
                            &proxy_info_tmp)) {
...
bool auto_detect_policy::check_policy(
    const WINHTTP_CURRENT_USER_IE_PROXY_CONFIG &config) {
...
void detect_proxy(session &http_session, const url &dest_url) {
  ...
  if (WinHttpGetIEProxyConfigForCurrentUser(&config)) {
    if (named_proxy_policy::check_policy(config)) {
      ...
      proxy_info =
          named_proxy_policy::make_proxy_info(http_session, dest_url, config);
    }
    if (auto_config_url_policy::check_policy(config)) {
      LOG(L"auto_config_url_policy");
      proxy_info = auto_config_url_policy::make_proxy_info(http_session,
                                                           dest_url, config);
    }
...
connection make_connection(session &http_session, const url &dest_url) {
  ...
  if (!(raw_handle = WinHttpConnect(http_session, dest_url.get_host().c_str(),
                                    get_port_from_url(dest_url), 0))) {
...
request make_request(connection &http_connection, const std::wstring &verb,
                     const url &dest_url) {
  HINTERNET raw_handle = NULL;
  DWORD flags = 0;
  ...
  if (!(raw_handle = WinHttpOpenRequest(
            http_connection, verb.c_str(), dest_url.get_path().c_str(), NULL,
...
buffer_t read_data(request &http_request, size_t size) {
  ...
  buffer.resize(size);
  if (!WinHttpReadData(http_request, (LPVOID)&buffer[0], size, &downloaded)) {
...
void set_credentials(request &http_request, DWORD target, DWORD auth_scheme,
                     const std::wstring &username,
                     const std::wstring &password) {
  if (!WinHttpSetCredentials(http_request, target, auth_scheme,
                             username.c_str(), password.c_str(), NULL)) {
...
std::tuple<DWORD, DWORD, DWORD> query_auth_scheme(request &http_request) {
  ...
  DWORD first_scheme;
  DWORD target;
  if (!WinHttpQueryAuthSchemes(http_request, &supported_schemes, &first_scheme,
                               &target)) {
...
buffer_t fetch_request(const url &dest_url, const std::wstring &username,
                       const std::wstring &password,
  ...
  // When proxy authentication is required begin the proxy authentication
  // challenge
  ...
  while (!done) {
    if (proxy_auth_scheme != 0) {
      ...
      set_credentials(http_request, WINHTTP_AUTH_TARGET_PROXY,
                      proxy_auth_scheme, username, password);
    }
    ...
    case 401:
      ...
      std::tie(supported_schemes, first_scheme, target) =
          query_auth_scheme(http_request);
      if (auto selected_scheme = select_auth_scheme(supported_schemes) != 0) {
        LOG(L"Sending server credentials");
        set_credentials(http_request, target, selected_scheme, username,
                        password);
      } else {
        done = true;
      }
