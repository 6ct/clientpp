...
Game::Game(const rapidjson::Value &data)
	: id(data[0].GetString(), data[0].GetStringLength()), region(region_id(std::string(data[1].GetString(), data[1].GetStringLength()))), map(std::string(data[4]["i"].GetString(), data[4]["i"].GetStringLength())), mode(data[4]["g"].GetInt()), players(data[2].GetInt()), max_players(data[3].GetInt()), custom(data[4]["c"].GetInt())
{
}
	...
	try
	{
		...
		for (const rapidjson::Value &data : data["games"].GetArray())
		{
			...
			if (!customs && game.custom)
				continue;
			if (use_map && Manipulate::lowercase(game.map) != map)
				continue;
