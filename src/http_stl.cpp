...
handle_manage_t::~handle_manage_t() {
  if (handle_ != nullptr)
    WH_INTERNET(CloseHandle)(handle_);
}

session_t::session_t(const std::string &user_agent) {
  std::wstring wide_user_agent =
      std::wstring(std::begin(user_agent), std::end(user_agent));
...
connection_t::connection_t(const session_t &sess, const std::string &host)
  ...
  std::wstring host_only(components_.lpszHostName,
                         components_.dwHostNameLength);
#ifdef WH_USE_WININET
  handle_ =
      InternetConnectW(sess.handle(), host_only.c_str(), components_.nPort,
                       nullptr, nullptr, INTERNET_SERVICE_HTTP, 0, 0);
#else
  handle_ =
      WinHttpConnect(sess.handle(), host_only.c_str(), components_.nPort, 0);
#endif
  if (handle_ == nullptr) {
...
response_t connection_t::send(const request_t &req) {
  ...
  if (WH_INTERNETW(CrackUrl)(req.url_.c_str(), 0, 0, &url_comps)) {
    ...
    if (url_comps.dwSchemeLength > 0 &&
        url_comps.nScheme != components_.nScheme) {
      THROW_LAST_ERROR("request_t url used a different scheme than the "
                       "connection_t was initialized with");
      return response_t(nullptr);
    }

    if (url_comps.dwHostNameLength > 0 &&
        _wcsnicmp(url_comps.lpszHostName, components_.lpszHostName,
                  url_comps.dwHostNameLength) != 0) {
      THROW_LAST_ERROR("request_t url used a different host name than the "
                       "connection_t was initialized with");
      return response_t(nullptr);
    }

    if (url_comps.nPort != components_.nPort) {
      THROW_LAST_ERROR("request_t url used a different port than the "
                       "connection_t was initialized with");
      return response_t(nullptr);
    }
  ...
  handle_manage_t request_t(WH_HTTPW(OpenRequest)(
      handle_, req.method_.c_str(), path.c_str(), nullptr, nullptr,
      accept_types, open_request_flags WH_WININET_ARGS(0)));
  ...
  if (!WH_INTERNET(SetOption)(request_t,
                              WH_INTERNET_CONST(OPTION_SECURITY_FLAGS),
                              (LPVOID)&security_flags, sizeof(DWORD))) {
    THROW_LAST_ERROR("WinHttpSetOption(WINHTTP_OPTION_SECURITY_FLAGS) on "
                     "request_t handle failed");
    return response_t(nullptr);
  }

  DWORD timeout = timeout_;
  if (!WH_INTERNET(SetOption)(request_t, WH_INTERNET_CONST(OPTION_SEND_TIMEOUT),
                              (LPVOID)&timeout, sizeof(DWORD))) {
    THROW_LAST_ERROR("WinHttpSetOption(WINHTTP_OPTION_SEND_TIMEOUT) on "
                     "request_t handle failed");
    return response_t(nullptr);
  }

  auto headers_end = std::end(req.additional_headers_);
  for (auto iter = std::begin(req.additional_headers_); iter != headers_end;
       ++iter) {
    if (!WH_HTTPW(AddRequestHeaders)(request_t, iter->c_str(), iter->length(),
                                     WH_HTTP_CONST(ADDREQ_FLAG_ADD) |
                                         WH_HTTP_CONST(ADDREQ_FLAG_REPLACE))) {
...
#ifdef WH_USE_WININET
  if (!HttpSendRequestW(request_t, nullptr, 0, (LPVOID)req.body_.c_str(),
                        total_request_length)) {
    THROW_LAST_ERROR("HttpSendRequest() failed");
    return response_t(nullptr);
  }
#else
  if (!WinHttpSendRequest(request_t, WINHTTP_NO_ADDITIONAL_HEADERS, 0,
                          WINHTTP_NO_REQUEST_DATA, 0, total_request_length,
                          0)) {
  ...
  if (total_request_length > 0) {
    ...
    if (!WinHttpWriteData(request_t, req.body_.data(), total_request_length,
                          &bytes_written)) {
...
request_t::request_t(const std::string &method, const std::string &url)
    : method_(std::begin(method), std::end(method)),
      url_(std::begin(url), std::end(url)), flags_(0) {}
...
response_t::response_t(HINTERNET request_t)
    : handle_manage_t(request_t), status_(-1), buffer_(nullptr) {
  if (handle_ != nullptr) {
#ifdef WH_USE_WININET
    char status_code[32];
    DWORD status_code_size = sizeof(status_code);
    DWORD header_index = 0;
    if (!HttpQueryInfoA(request_t, HTTP_QUERY_STATUS_CODE, &status_code,
                        &status_code_size, &header_index)) {
    ...
    if (!WinHttpQueryHeaders(
            request_t, WINHTTP_QUERY_STATUS_CODE | WINHTTP_QUERY_FLAG_NUMBER,
...
response_t::response_t(response_t &&other)
    : handle_manage_t(other.handle_), status_(other.status_),
...
bool response_t::read(std::ostream &out) {
  ...
  while (true) {
    ...
    if (!WH_INTERNET(QueryDataAvailable)(
            handle_, &data_available WH_WININET_ARGS(0, 0))) {
    ...
    while (data_available > 0) {
      DWORD bytes_read;
      DWORD chunk_size =
          read_buffer_size < data_available ? read_buffer_size : data_available;
