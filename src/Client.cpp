...
bool Client::navigation_cancelled(ICoreWebView2 *sender, Uri uri)
{
	...
	if (kru_owns)
	{
		if (uri.host_owns(L"docs.krunker.io") || pathname.starts_with(krunker::docs))
			send = &documents;
...
void Client::install_runtimes()
{
	if (MessageBox(NULL, L"You are missing runtimes. Install the WebView2 Runtime?", title, MB_YESNO) == IDYES)
	{
		...
		MessageBox(NULL, L"Cannot continue without runtimes. Client will now exit.", title, MB_OK);
}

Client::Client(HINSTANCE h, int c)
	: inst(h), cmdshow(c), updater(version, L"https://6ct.github.io/serve/updates.json"), installer(L"https://go.microsoft.com/fwlink/p/?LinkId=2124703"), folder(L"GC++"), game(
																																												folder, KrunkerWindow::Type::Game, {0.8, 0.8}, title, [this]()
	  ...
	  social(
		  folder, KrunkerWindow::Type::Social, {0.7, 0.7}, (std::wstring(title) + L": Social").c_str(), [this]()
		  { listen_navigation(social); },
		  [this](JSMessage msg) -> bool
		  { return on_message(msg, social); }),
	  editor(
		  folder, KrunkerWindow::Type::Editor, {0.7, 0.7}, (std::wstring(title) + L": Editor").c_str(), [this]()
		  { listen_navigation(editor); },
		  [this](JSMessage msg) -> bool
		  { return on_message(msg, editor); }),
	  scripting(
		  folder, KrunkerWindow::Type::Scripting, {0.6, 0.6}, (std::wstring(title) + L": Scripting").c_str(), [this]()
		  { listen_navigation(scripting); },
		  [this](JSMessage msg) -> bool
		  { return on_message(msg, scripting); }),
	  documents(
		  folder, KrunkerWindow::Type::Documents, {0.4, 0.6}, (std::wstring(title) + L": Documents").c_str(), [this]()
		  { listen_navigation(documents); },
		  [this](JSMessage msg) -> bool
		  { return on_message(msg, documents); }),
	  accounts(folder), shcore(LoadLibrary(L"api-ms-win-shcore-scaling-l1-1-1.dll"))
{
}
...
bool Client::create()
{
	...
	if (!SUCCEEDED(coinit))
		MessageBox(NULL, (L"COM could not be initialized. CoInitialize returned " + Convert::wstring(std::to_string(coinit))).c_str(), title, MB_OK);
	...
	if (shcore)
	{
		...
		if (std::function SetProcessDpiAwareness = (dec *)GetProcAddress(shcore, "SetProcessDpiAwareness"))
		{
			...
			if (!SUCCEEDED(sda))
				clog::error << "SetProcessDpiAwareness returned 0x" << std::hex << HRESULT_CODE(sda) << clog::endl;
		}
		else
			clog::error << "Unable to get address of SetProcessDpiAwareness" << clog::endl;
	}
