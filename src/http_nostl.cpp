...
handle_manager_t::~handle_manager_t() {
  if (handle_ != nullptr)
...
connection_t::connection_t(const session_t &sess, const char *host)
    : flags_(0), timeout_(30), host_(alloc_wide_string(host)) {
  ...
  wcsncpy_s(host_only, buffer_bytes, components_.lpszHostName,
            components_.dwHostNameLength);
#ifdef WH_USE_WININET
  handle_ = InternetConnectW(sess.handle(), host_only, components_.nPort,
...
connection_t::~connection_t() { delete[] host_; }
...
response_t connection_t::send(const request_t &req) {
  ...
  if (WH_INTERNETW(CrackUrl)(req.url_, 0, 0, &url_comps)) {
    ...
    if (url_comps.dwSchemeLength > 0 &&
      ...
      set_error("request_t url used a different scheme than the connection_t "
    ...
    if (url_comps.dwHostNameLength > 0 &&
      ...
      set_error("request_t url used a different host name than the "
                "connection_t was initialized with");
      return response_t(nullptr);
    }

    if (url_comps.nPort != components_.nPort) {
      set_error("request_t url used a different port than the connection_t was "
                "initialized with");
      return response_t(nullptr);
    }
  ...
  handle_manager_t request_t(WH_HTTPW(OpenRequest)(
      handle_, req.method_, path, nullptr, nullptr, accept_types,
      open_request_flags WH_WININET_ARGS(0)));
  ...
  if (!WH_INTERNET(SetOption)(request_t,
    ...
    set_error("WinHttpSetOption(WINHTTP_OPTION_SECURITY_FLAGS) on request_t "
              "handle failed");
    return response_t(nullptr);
  }
  ...
  if (!WH_INTERNET(SetOption)(request_t, WH_INTERNET_CONST(OPTION_SEND_TIMEOUT),
    ...
    set_error("WinHttpSetOption(WINHTTP_OPTION_SEND_TIMEOUT) on request_t "
              "handle failed");
    return response_t(nullptr);
  }
  ...
  while (entry != nullptr) {
    if (!WH_HTTPW(AddRequestHeaders)(request_t, entry->line_,
                                     lstrlenW(entry->line_),
...
#ifdef WH_USE_WININET
  if (!HttpSendRequestW(request_t, nullptr, 0, req.body_,
                        total_request_length)) {
    set_error("HttpSendRequest() failed");
    return response_t(nullptr);
  }
#else
  if (!WinHttpSendRequest(request_t, nullptr, 0, nullptr, 0,
  ...
  if (total_request_length > 0) {
    ...
    if (!WinHttpWriteData(request_t, req.body_, total_request_length,
                          &bytes_written)) {
...
request_t::request_t(const char *method, const char *url)
    : method_(alloc_wide_string(method)), url_(alloc_wide_string(url)),
      body_(nullptr), body_length_(0), flags_(0), headers_head_(nullptr),
...
response_t::response_t(HINTERNET request_t)
    : handle_manager_t(request_t), status_(-1) {
  if (handle_ != nullptr) {
#ifdef WH_USE_WININET
    char status_code[32];
    DWORD status_code_size = sizeof(status_code);
    DWORD header_index = 0;
    if (!HttpQueryInfo(request_t, HTTP_QUERY_STATUS_CODE, &status_code,
...
response_t::response_t(response_t &&other)
    : handle_manager_t(other.handle_), status_(other.status_) {
...
bool response_t::read(char *buffer, size_t count, size_t *bytes_read) {
  ...
  while (remaining > 0) {
    ...
    while (data_available > 0 && remaining > 0) {
      DWORD copied;
      DWORD chunk_size =
          data_available < remaining ? data_available : remaining;
