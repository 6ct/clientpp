...
rapidjson::Document KrunkerWindow::get_userscript_schema() {
  ...
  rapidjson::ParseResult ok =
      schema_doc.Parse(schema_raw.data(), schema_raw.size());

  if (!ok)
    clog::error << "Error parsing userscript schema: "
  ...
  for (IOUtil::WDirectoryIterator it(folder.directory + folder.p_scripts,
    ...
    if (IOUtil::read_file(it.path().c_str(), buffer)) {
      ...
      if (std::regex_search(buffer, match, meta_const)) {
        std::string jsonMatch =
            std::regex_replace(match.str(1), meta_comment, "");

        rapidjson::ParseResult ok =
            metadata.Parse(jsonMatch.data(), jsonMatch.size());

        if (!ok) {
          errors.push_back(std::string("Error parsing userscript: ") +
                           GetParseError_En(ok.Code()) + " (" +
                           std::to_string(ok.Offset()) + ")");
          clog::info << "Here: " << jsonMatch.substr(ok.Offset() - 10, 20)
                     << clog::endl;
        } else {
          ...
          if (!metadata.Accept(validator)) {
            ...
            errors.push_back(std::string("Invalid keyword: ") +
                             validator.GetInvalidSchemaKeyword());
            sb.Clear();
            validator.GetInvalidDocumentPointer().StringifyUriFragment(sb);
            errors.push_back(std::string("Invalid document: ") +
                             sb.GetString());
          }

          if (metadata.HasMember("features")) {
            if (metadata["features"].HasMember("block"))
              additional_block_patterns.push_back(
                  std::wregex(JT::wstring(metadata["features"]["block"])));

            if (metadata["features"].HasMember("command_line"))
              for (rapidjson::Value::ValueIterator it =
                       metadata["features"]["command_line"].Begin();
                   it != metadata["features"]["command_line"].End(); ++it)
                additional_command_line.push_back(JT::wstring(*it));

            if (metadata["features"].HasMember("config")) {
              bool changed = false;

              rapidjson::Document::AllocatorType folder_allocator =
                  folder.config.GetAllocator();

              if (!folder.config["userscripts"].HasMember(
                      rapidjson::Value(metadata["author"], folder_allocator))) {
                folder.config["userscripts"].AddMember(
                    rapidjson::Value(metadata["author"], folder_allocator),
                    rapidjson::Value(metadata["features"]["config"],
                                     folder_allocator),
                    folder_allocator);
                changed = true;
              } else {
                ...
                folder.config["userscripts"][metadata["author"]] =
                    rapidjson::Value(
                        TraverseCopy(
                            folder.config["userscripts"][metadata["author"]],
                            metadata["features"]["config"], folder_allocator,
                            true, &changed),
                        folder_allocator);
              }
      ...
      for (std::string error : errors) {
        ...
        errors_array.PushBack(
            rapidjson::Value(error.data(), error.size(), allocator), allocator);
      }
...
rapidjson::Value KrunkerWindow::load_css(
  ...
  if (load_resource(CSS_CLIENT, cli_css)) {
    ...
    row.PushBack(rapidjson::Value(cli_css.data(), cli_css.size(), allocator),
                 allocator);
    result.PushBack(row, allocator);
  } else
