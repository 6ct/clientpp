    ...
    if (IOUtil::read_file(it.path().c_str(), buffer)) {
      ...
      if (std::regex_search(buffer, match, meta_const)) {
        ...
        try {
          ...
          if (raw_metadata.contains("features"))
            raw_features = raw_metadata["features"];
          if (raw_features.contains("gui"))
            features["gui"] = raw_features["gui"];
          if (raw_features.contains("config"))
            features["config"] = raw_features["config"];

          JSON config = features["config"];

          for (std::string host : features["block_hosts"])
            add_back<std::wstring>(additional_block_hosts,
                                   Convert::wstring(host));
          ...
          if (!userscripts.contains(author)) {
            ...
            userscripts[author] =
                TraverseCopy(userscripts[author], config, true, &changed);
          if (changed) {
          ...
          errors.push_back("Unable to read metadata from userscript " +
                           Convert::string(it.file()) + ": " + err.what());
        } catch (JSON::parse_error err) {
          errors.push_back("Unable to process metadata from userscript " +
                           Convert::string(it.file()) + ": " + err.what());
        }
...
JSON KrunkerWindow::runtime_data() {
  ...
  if (load_resource(CSS_CLIENT, cli_css))
    data["css"]["client/builtin.css"] = cli_css;
  else
