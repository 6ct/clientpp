...
std::wstring Uri::host(bool remove_www) {
  ...
  if (remove_www && host.starts_with(L"www."))
    host = host.substr(4);

  size_t first_slash = host.find_first_of(L'/');
  if (first_slash != std::wstring::npos)
    host = host.substr(0, first_slash);

  return host;
}

std::wstring Uri::origin() { return protocol() + L"//" + host(); }

std::wstring Uri::path() { return href.substr(origin().length()); }

std::wstring Uri::search() {
  std::wstring p = path();
  size_t first = p.find_first_of(L'?');
  if (first != std::wstring::npos)
    return p.substr(first);
  else
    return L"";
}
