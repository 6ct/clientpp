...
template <typename Tag> class internet_handle {
public:
  ...
  internet_handle &
  operator=(internet_handle &&other) noexcept // move assignment
  {
...
struct named_proxy_policy {
  ...
  make_proxy_info(session &http_session, const url &dest_url,
...
buffer_t fetch_request(
    const url &dest_url, const std::wstring &username = L"",
    const std::wstring &password = L"",
    const std::wstring &user_agent = L"Mozilla/5.0 (Windows NT 6.1; WOW64) "
